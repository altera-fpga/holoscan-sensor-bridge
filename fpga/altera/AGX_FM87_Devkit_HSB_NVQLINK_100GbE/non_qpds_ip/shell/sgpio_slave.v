...
module sgpio_slave (
  input  wire       i_rstn,
  output wire [7:0] o_user_sw,        //the value of user_sw that slave gets from master
  input  wire [7:0] i_user_led,       //the value of user_led that slave intends to drive
  output wire       o_user_sw_valid,  //=1: o_user_sw is the active value driven by master
  output wire       o_miso,           //master input slave output of the synchronized serial interface
  input  wire       i_clk,            //clock of the synchronized serial interface
  input  wire       i_sync,           //frame start of the synchronized serial interface, =1: the start of a frame
  input  wire       i_mosi            //master output slave input of the synchronized serial interface
);
