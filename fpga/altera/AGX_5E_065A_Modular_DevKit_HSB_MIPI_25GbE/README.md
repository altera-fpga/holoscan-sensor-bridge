# Holoscan Sensor Bridge IMX678 MIPI to 25GbE System Example Design for Agilex™ 5 Group A Devices

## Overview

The Holoscan Sensor Bridge IMX678 MIPI to 25GbE System Example Design for Agilex™ 5 Group A Devices demonstrates an implementation of using industry-standard Mobile Industry Processor Interface (MIPI) D-PHY and MIPI CSI-2 interface on Agilex™ 5 Group A FPGAs to integrate to a Holoscan processing flow.
...
The FPGA design comprises a MIPI D-PHY and two MIPI CSI-2 interfaces connected to the NVIDIA Holoscan Sensor Bridge IP and the Altera® GTS Ethernet Hard IP. The design incorporates a VVP Broadcaster per camera to simulate a total of eight simultaneous sources.
...
<p align="center">
<img src="./assets/HSB_MIPI_25GbE_Overview.png" alt="Block diagram showing the HSB MIPI to 25GbE system architecture with MIPI camera inputs connecting through D-PHY and CSI-2 interfaces to FPGA fabric, replicating the streams then through Holoscan Sensor Bridge IP and GTS Ethernet IP to network output"><br>
...
The following example applications are available to demonstrate the Holoscan Sensor Bridge IMX678 MIPI to 25GbE System Example Design for Agilex™ 5 Devices.
...
* 8 Stream Playback Example

```bash
python3 examples/agx5_multiviewer.py --cam 8 --frame-rate 30
```

> [!NOTE]
> The user is responsible for ensuring the multiviewer combination does not exceed the available link bandwidth. 
...
This project contains the necessary files and collateral to build the Holoscan IMX678 Sensor Bridge MIPI to 25GbE System Example Design for Agilex™ 5 Devices. It has been built using Altera® Quartus® Prime Pro Edition version 26.1.
...
<p align="center">
<img src="assets/MDK-25GbE-pd_top.png" alt="Dual MIPI Ingest with broadcast Block Diagram"><br>
...
The Thor AGX system supports Camera Over Ethernet hardware acceleration and a hardened ISP. The FPGA design supports these features however only Camera Over Ethernet has been tested.
...
└── fpga
    ├── nv_hsb_ip 
    └── altera
        └── AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE
```
...
```
quartus
├── AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.qsf                   # Main project QSF, calls submodule QSFs
├── shell
│   ├── AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE_supplemental.qsf  # Additional project settings
...
└── user
    ...
    ├── mipi_hsb_broadcast_subsystem_0.qsf                          # MIPI HSB broadcast subsystem 0 source assignments
    ├── mipi_hsb_broadcast_subsystem_1.qsf                          # MIPI HSB broadcast subsystem 1 source assignments
...
### Design Hierarchy
```
AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.v (Project Top Level)
    └── AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE_qsys.qsys (Top Level Platform Designer)
            ├── board_subsystem.qsys
            ├── clock_subsystem.qsys
            ├── gts_eth_subsystem.qsys
            ├── hsb_subsystem.qsys
            ├── mipi_hsb_broadcast_subsystem_0.qsys
            ├── mipi_hsb_broadcast_subsystem_1.qsys
            └── mipi_subsystem.qsys
```
...
| GTS Ethernet Clock | GTS IP | 402.83203125 | Host Interface | 
| Core Video Clock | IOPLL (Board clock source) | 400 | Video pipeline and Sensor Interface | 
...
#### `mipi_hsb_broadcast_subsystem_*` (user)

**Role:** Converts from [Altera® Streaming Video Protocol (ASVP)](https://docs.altera.com/r/docs/683397/current/altera-streaming-video-protocol-specification/about-the-altera-streaming-video-protocol) to the packed CSI data format that is expected by the HSB IP and software. Instantiates a VVP broadcaster to replicate the video stream.

**Functions:**

- **CSI packer** for aligned data packing.
- **VVP Broadcaster** for pipeline replication with internal FIFOs for line buffering.
...
|Board|16|32|0|0|
|Clock|12|87|0|0|
|GTS Ethernet|1024|1463|1|0|
|HSB|73201|184440|50|4|
|MIPI HSB Broadcast 0|1135|3435|28|0|
|MIPI HSB Broadcast 1|1135|3308|28|0|
|MIPI|10004|21450|23|0|
|***Total***|***86527***|***214215***|***130***|***4***|

> [!NOTE]
> The logic utilisation in this design is greatly increased by enabling [Camera over Ethernet](#camera-over-ethernet) support. For this design the increase is approximately 56k ALMs (8x Video SIFs x 7k ALMs) 

---

### Building the Design

The example design can be built in two ways, using a build script or opening the project in Quartus® and building using the GUI. Under both methods the built FPGA bitstream `AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.sof` is located in the `quartus/output_files` directory.
<br>

#### Script build

- Navigate to the `AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE/scripts` directory:
...
- Navigate to the `AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE/quartus/output_files` directory:

```bash
quartus_pfg -c -o device=QSPI02G -o mode=ASX4 -o flash_loader=A5ED065AB32AE1V AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.sof AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.jic
```
...
* [Agilex™ 5 FPGA and SoC E-Series 065A Modular Development Kit (Group A Device) - MK-A5E065AB32AEA](https://www.altera.com/products/devkit/po-3278/agilex-5-fpga-and-soc-e-series-065a-modular-development-kit)

<br>

<p align="center">
<img src="assets/Agx5A-MDK.png" alt="Agx5A-MDK"><br>
<strong>Agilex™ 5 FPGA E-Series 065A Modular Development Kit</strong>
</p>
...
* [25GBase-T SFP28 to QSFP28 Direct Attach cable](https://www.mouser.co.uk/ProductDetail/Amphenol-ProLabs/QFX-QSFP28-SFP28-DAC-2M-C?qs=3vio67wFuYougL76tAlILg%3D%3D).
* NVIDIA Platform:
...
<p align="center">
<img src="./assets/Agx5A-MDK-ed-conn.png" alt="ed-conn"><br>
...
> [!NOTE]
> The screenshots in this section have been captured using the Group B Modular Developmet Kit.
> For the Group A MDK the JTAG hardware will show as SM72+ instead of SM72 and the FPGA part number will be A5ED065AB32A.
...
* To program the FPGA using SOF:
  ...
  * Either use your own or download the pre-built `SOF` image, and program
    ...
    ```bash
    quartus_pgm -c 1 -m jtag -o "p;AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.sof"
    ```
  ...
  * or, optionally use the Quartus® Programmer GUI:
    ...
    * Select your `AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.sof` file. Check the **"Program/Configure"** box and press the **"Start"** button (see below). Wait until the programming has been completed.
...
* To program the QSPI flash using JIC:
  ...
  * Either use your own or download the pre-built `JIC` image, and program
    ...
    ```bash
    quartus_pgm -c 1 -m jtag -o "pvi;AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.jic"
    ```
  ...
  * or, optionally use the Quartus® Programmer GUI:
    ...
    * Select your `AGX_5E_065A_Modular_DevKit_HSB_MIPI_25GbE.jic` file. Note that a QSPI device may have newly appeared in the chain.
...
* When using multiple streams with Camera Over Ethernet on Thor the underlying data handling application can crash preventing Ethernet packets from being correctly processed on the Thor's QSFP connection. A reboot of the Thor is required.