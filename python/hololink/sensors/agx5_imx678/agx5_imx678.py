...
class FramosImx678:
    ...
    def _hsb_raw_line_bytes(self):
        if self._pixel_format == hololink_module.sensors.csi.PixelFormat.RAW_10:
            return self._width * 5 // 4
        if self._pixel_format == hololink_module.sensors.csi.PixelFormat.RAW_12:
            return self._width * 3 // 2
        if self._pixel_format == hololink_module.sensors.csi.PixelFormat.RAW_8:
            return self._width
        raise Exception(f"Unsupported pixel format: {self._pixel_format}")

    def _packetizer_wire_line_bytes(self, hsb_raw_line_bytes):
        if self._pixel_format == hololink_module.sensors.csi.PixelFormat.RAW_10:
            # CEILING(hsb_raw / 120) * 128
            return ((hsb_raw_line_bytes + 119) // 120) * 128
        if self._pixel_format == hololink_module.sensors.csi.PixelFormat.RAW_12:
            # CEILING(CEILING(hsb_raw, 24), 64)
            return hololink_module.round_up(
                hololink_module.round_up(hsb_raw_line_bytes, 24), 64
            )
        return hololink_module.round_up(hsb_raw_line_bytes, 64)
    ...
    def configure_converter(self, converter):
        ...
        if type(converter).__name__ == "FusaCoeCaptureOp":
            # Temporary: match observed packetizer wire bytes (e.g. 1792 for 720p RAW10)
            # instead of round_up(COE logical, 64) which yields 1728.
            hsb_raw = self._hsb_raw_line_bytes()
            received_line_bytes = self._packetizer_wire_line_bytes(hsb_raw)
            logging.info(
                "FUSA COE wire line bytes: hsb_raw=%d, bytes_per_line=%d (COE logical=%d)",
                hsb_raw,
                received_line_bytes,
