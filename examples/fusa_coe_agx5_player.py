...
class App(holoscan.core.Application):
    ...
    def compose(self):
        ...
        logging.info("Using AGX5 IMX678:")
...
def main():
    ...
    # configure the default lines based on the board type
    if channel_metadata["fpga_uuid"] == a_mdk_uuid:
        default_lines = 2160  # Friendly for 25G connections
    else:
        default_lines = 1080  # Friendly for 10G connections
    ...
    for i in range(len(hololink_channels)):
        ...
        camera = hololink_module.sensors.agx5_imx678.agx5_imx678.FramosImx678(
            hololink_channel=hololink_channels[i],
            camera_id=i % 2 if args.sensor == -1 else args.sensor,
        )

        # Configures the camera for 1920x1080, 60fps, 10bits per pixel
        camera.configure(camera.get_mode(args.lines, args.frame_rate, args.bit_depth))
        cameras.append(camera)

    # We are not going to update the camera_mode during application setup
    camera_mode = None
